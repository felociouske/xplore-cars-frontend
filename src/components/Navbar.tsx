...
// Added `external` + `href` — internal links keep using `path` with
// react-router's <Link>; the calculator link uses `href` with a plain
// <a>, since it's a genuinely different domain (tax.xplorecars.cc).
const navLinks: { name: string; path?: string; hash?: string; external?: boolean; href?: string }[] = [
  { name: "Home", path: "/" },
...
const Navbar = () => {
  ...
  return (
    <>
              ...
              {navLinks.map((link) =>
                link.external ? (
                  <a
                    ...
                    className="relative px-4 py-2 rounded-md text-sm font-body font-medium text-foreground hover:text-accent transition-colors after:absolute after:left-4 after:-bottom-0.5 after:h-[2px] after:bg-accent after:transition-all after:duration-300 after:w-0 hover:after:w-[calc(100%-2rem)]"
                  >
                    {link.name}
                  </a>
                ) : (
      ...
      <div className="fixed inset-0 z-[100] md:hidden pointer-events-none" aria-hidden={!isOpen}>
        <div
                ...
                >
                  {link.external ? (
                    <a
                      ...
                      rel="noopener noreferrer"
                      onClick={() => setIsOpen(false)}
                      className="flex items-center justify-between px-4 py-3.5 rounded-xl text-sm font-semibold text-foreground hover:bg-secondary transition-colors group"
                    >
                        <span>{link.name}</span>
